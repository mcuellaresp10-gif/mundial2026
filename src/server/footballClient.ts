...
/** Goleadores del Mundial (API players/topscorers). */
export async function getWorldCupTopScorers(season = DEFAULT_SEASON) {
  const rows = await apiGetCached<Player[]>(
    "players/topscorers",
    { league: LEAGUE_ID, season },
    TTL_PLAYERS_MS
  );
  return mapPlayersToTopScorers(rows);
}

/** Página de jugadores con stats del Mundial (league=1). */
export async function getWorldCupPlayersPage(
  page = 1,
  ...
  const url = `${getApiBaseUrl()}/players?${qs}`;
  const key = cacheKey("players", { league: LEAGUE_ID, season, page });
  const cached = getCached<{ players: Player[]; paging: { current: number; total: number } }>(
    key,
    TTL_PLAYERS_MS
  );
  ...
  const json = (await res.json()) as ApiResponse<Player[]>;
  const result = {
