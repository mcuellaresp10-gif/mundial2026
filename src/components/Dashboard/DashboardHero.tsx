...
export function DashboardHero() {
  return (
    <header
      id="inicio"
      className="scroll-mt-24 @container/hero mb-6 lg:mb-8"
    >
        ...
        <div className="relative grid gap-6 @md/hero:grid-cols-[1fr_auto] @md/hero:items-end min-w-0">
          <div className="min-w-0">
            ...
            <p className="text-white/70 mt-2 max-w-prose text-sm @md/hero:text-base">
              Análisis táctico y estadísticas en vivo
            </p>
          </div>
          ...
          <div className="hidden @md/hero:flex flex-col items-end gap-1 text-right shrink-0">
            ...
            <span className="font-mono text-2xl font-bold text-mundial-gold">2026</span>
          </div>
        </div>
        ...
        >
          {QUICK_LINKS.map(({ href, label, icon: Icon }) => (
            <Link
              ...
              href={href}
              className={cn(
                "group flex min-w-0 items-center gap-2 rounded-xl border border-white/10",
                "bg-white/5 px-3 py-2.5 text-sm font-medium text-white/90",
                "transition-colors hover:bg-white/10 hover:border-mundial-gold/40 hover:text-white"
              )}
            >
              <Icon className="h-4 w-4 shrink-0 text-mundial-gold/80 group-hover:text-mundial-gold" />
